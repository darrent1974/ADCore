...
#define str_NDFileHDF5_reuseContainer "HDF5_reuseContainer"
#define str_NDFileHDF5_insertionOrigin "HDF5_insertionOrigin"
...
protected:
  /* plugin parameters */
  int NDFileHDF5_reuseContainer;
  int NDFileHDF5_insertionOrigin;
...
private:
  ...
  hid_t createFileAccessPlist();
  hid_t createFileCreatePlist();
  asynStatus createNewFile(const char *fileName);
  asynStatus openExistingFile(const char *fileName);
  int getGroupPathNames(const char *groupPath, std::vector<std::string> &vecNames);
  asynStatus initialiseInsertionOrigin(const char *insertionOrigin);
